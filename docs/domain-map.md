...
| Source Export Integration | DOCUMENTED WITHOUT IMPLEMENTATION | source products own export; platform owns consumption | No platform profile copy; checkpoint state not yet implemented | fetch/reconcile/import approved source records | export page/checkpoint status | Party-reference events only after accepted changes | Party References; authenticated source contract |
